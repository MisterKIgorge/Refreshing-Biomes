...
local actionhandlers = 
{
    ...
    ActionHandler(ACTIONS.SWARMATTACH,
        function(inst, action)
            return action and action.target and action.target:HasTag("lilypad") and inst:HasTag("wanttococoon") and "spawncocoon" or "infest"
        end),
...
local states=
{
    ...
    State{
        ...
        onenter = function(inst)
			if inst.data and inst.data.SCALE ~= 0.5 then
				inst:SplitSwarms()
			else
    ...
    State{
        ...
        {
            EventHandler("animqueueover", function(inst)
				inst.plantedcocoon = true
				inst:RemoveTag("wanttococoon")
				SpawnAt("mosquitoswarm_cocoon", inst)
...
return StateGraph("mosquitoswarm", states, events, "spawn", actionhandlers)
